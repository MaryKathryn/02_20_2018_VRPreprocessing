function [taskStruct] = ALVR_PrepALFixedData(dirDataSessTask,bSave)
...
dirFiles  = dir(dirDataSessTask);
[~,dirFilesOrder] = sort([dirFiles.datenum]);
dirFiles  = dirFiles(dirFilesOrder);

% Gets valid trials (non-empty trials):
trlNames = {dirFiles(cell2mat(cellfun(@(x)(~isempty(x)), ...
    (strfind({dirFiles.name}, 'Trial')), 'uni', false))).name};
EDFFile = {dirFiles(cell2mat(cellfun(@(x) (~isempty(x)), ...
     (strfind({dirFiles.name}, 'edf')), 'uni', false))).name};
...
if ~strcmp('el.edf',EDFFile)&~isempty(EDFFile)
    edfEyeSamplefile = edfmex(char([dirDataSessTask sl EDFFile{1}] ));
else
    edfEyeSamplefile = [];
end
...
for trl = 1:numel(trlNames)
    ...
    load(char([dirDataSessTask sl trlNames{trl}])); %** correction made here **
...
%**                 
    ...
    if ~strcmp(Trl_TrialID,'000000000000000')
        ...
        taskStruct.SessionInfo.Context1NParams = trlParams.Context1N; % (1)
        taskStruct.SessionInfo.Context1SParams = trlParams.Context1S; % (1)
        taskStruct.SessionInfo.Context2NParams = trlParams.Context2N; % (1)
        taskStruct.SessionInfo.Context2SParams = trlParams.Context2S; % (1)
        ...
        taskStruct.Trials.(['ID_' Trl_TrialID]).TrlNumberA     = Trl_numberA;         % (2)**
        taskStruct.Trials.(['ID_' Trl_TrialID]).TrlNumberB     = Trl_numberB;         % (2)**
        taskStruct.Trials.(['ID_' Trl_TrialID]).Context        = Task_Context;        % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).ContextName    = Task_Cue;            % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).ContextMat     = Task_CueMat;         % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).Direction      = Task_Direction;      % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).TrainingPhase  = Task_TrainingPhase;  % (1)
        taskStruct.Trials.(['ID_' Trl_TrialID]).Blocks         = Trials_block;        % (1)**
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalWestColor  = Task_GoalWest;       % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalWestRew    = Task_GoalWestRew;    % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalWestHier   = Task_GoalWestHier;   % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalEastColor  = Task_GoalEast;       % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalEastRew    = Task_GoalEastRew;    % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalEastHier   = Task_GoalEastHier;   % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalLeft       = Task_GoalLeft;       % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).GoalRight      = Task_GoalRight;      % (2)
        taskStruct.Trials.(['ID_' Trl_TrialID]).OutcomeCorr    = Task_OutcomeCorr;    % (3)
        taskStruct.Trials.(['ID_' Trl_TrialID]).OutcomeColor   = Task_OutcomeColor;   % (3)
        taskStruct.Trials.(['ID_' Trl_TrialID]).OutcomeHier    = Task_OutcomeHier;    % (3)
        taskStruct.Trials.(['ID_' Trl_TrialID]).OutcomeDirec   = Task_OutcomeDirec;   % (3)
        taskStruct.Trials.(['ID_' Trl_TrialID]).OutcomeSide    = Task_OutcomeSide;    % (3)
    ...
    taskStruct.TrialSummary(trl+1,:) = {['ID_' Trl_TrialID] Task_Direction Task_TrainingPhase...
...
for trl = 1:length(trlIDs)
    ...
    % GoalsOnset Time:
    goalsAppearInd = find(strcmp(taskStruct.Trials.(trlIDs{trl}).Unreal_States,...
        ['Goals' taskStruct.Trials.(trlIDs{trl}).Direction 'On']),1, 'first');
    taskStruct.Trials.(trlIDs{trl}).GoalsOnset = ...
        taskStruct.Trials.(trlIDs{trl}).Unreal_Times(goalsAppearInd);
    ...
    if strcmp(taskStruct.Trials.(trlIDs{trl}).Direction, 'North')
        if any(taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(:,1) >= (-703-34))
            
            taskStruct.Trials.(trlIDs{trl}).ContextOnset = taskStruct.Trials.(trlIDs{trl}).Unreal_Times(find(taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(:,1) >= (-703-34), 1, 'first'));
            %
            %             goalsAppearInd  = ...
            %                 find(HierStruct.Trials.(trlIDs{trl}).PositionMatrix_deg(:,1) ...
            %                 >= 768, 1, 'first');
        else
            taskStruct.Trials.(trlIDs{trl}).ContextOnset = [];
        end
    elseif strcmp(taskStruct.Trials.(trlIDs{trl}).Direction, 'South')
        if any(taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(:,1) <= (704+48))
            
            taskStruct.Trials.(trlIDs{trl}).ContextOnset = taskStruct.Trials.(trlIDs{trl}).Unreal_Times(find(taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(:,1) <= (704+48), 1, 'first'));
    ...
    goalsRotTime = [];
    if ~isempty(goalsAppearInd) && (strcmp(taskStruct.Trials.(trlIDs{trl}).OutcomeCorr, 'Correct')||...
            strcmp(taskStruct.Trials.(trlIDs{trl}).OutcomeCorr, 'Incorrect'))
        
        %Find the first sample where the rotation crosses the threshold
        RotThreshInd = find(abs(...
            taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(goalsAppearInd:end,3)-...
            taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(goalsAppearInd,3)) > 10,...
            1, 'first')+goalsAppearInd;
        
        %Go back to the first sample where the rotation was initiated
        RotStartInd = find(...
            taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(goalsAppearInd:RotThreshInd,3) == ...
            taskStruct.Trials.(trlIDs{trl}).Unreal_PosXPosYRot(goalsAppearInd,3),1,'last')+goalsAppearInd;
        
        goalsRotTime = taskStruct.Trials.(trlIDs{trl}).Unreal_Times(RotStartInd) ;
    end
    taskStruct.Trials.(trlIDs{trl}).DecisionOnset = goalsRotTime;
end
...
if bSave
    ...
    taskStruct.SessionInfo.FileName = [monkeyName(1) '_' session '_XMaze.mat'];
    save([dirResultsSession XMazeStruct.SessionInfo.FileName], 'XMazeStruct');
