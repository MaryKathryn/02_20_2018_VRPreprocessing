function [FreeRoamStruct] = HcTask_PrepFreeRoamData(SessionDir)
...
hWaitBar = waitbar(0, 'Going through FreeRoam trials.');
...
for trl = 1:numel(ValidTrials)
    ...
    if ~strcmp(Trl_TrialID,'000000000000000')
        ...
        FreeRoamStruct.SessionInfo.MonkeyName      = Ses_MonkeyName;      % (8)
        FreeRoamStruct.SessionInfo.Date            = Ses_Date;            % (8)
        FreeRoamStruct.SessionInfo.SetupID         = Sys_SetupID;         % (8)
        FreeRoamStruct.SessionInfo.Goals           = Task_Goals;          % (1)
        
        %If no eye calibration in last trial, use the last valid values
        if ~isempty(Eye_Calibration)
            FreeRoamStruct.SessionInfo.EyeCalibration  = Eye_Calibration;     %
            FreeRoamStruct.SessionInfo.QuadrantCorrect = QuadrantCorrect;     %
        end
        
        FreeRoamStruct.SessionInfo.ScreenHeight    = Eye_ScreenHeight;
        FreeRoamStruct.SessionInfo.ScreenWidth     = Eye_ScreenWidth;
        FreeRoamStruct.SessionInfo.ScreenDistance  = Eye_ScreenDistance;
        
        % Trial-specific information
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).GoalID         = Task_GoalID;      % (2)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).Unreal_States  = Unreal_States;       % (3)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).Unreal_PosXPosYRot = Unreal_PosRots;  % (3)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).Unreal_Times   = Unreal_Times;        % (3)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).EyeSamplesBad  = Eye_SamplesBad;         %
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).EyeSamples    = Eye_Samples;         %
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).ITC18StartTime = Sync_ITC18StartTime; % (10)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).SOT_Time       = Trl_SOT;             % (6)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).EOT_Time       = Trl_EOT;             % (7)
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).SyncPulse      = Sync_Pulse;          %
        FreeRoamStruct.Trials.(['ID_' Trl_TrialID]).OutcomeWord    = Trl_Outcome;
        
    end
    
    waitbar(trl/numel(ValidTrials));
    display(['FreeRoam Trial ' Trl_TrialID(end-2:end) ' processing time : ' num2str(toc, '%3.3f') ' seconds']);
    
end

close(hWaitBar);
% sometimes the order gets screwed up when copying files over
FreeRoamStruct.Trials = orderfields(FreeRoamStruct.Trials);
FreeRoamStruct = HcTask_FormatEyeDataOffScreen(FreeRoamStruct, 1); % Modified by Ben Corrigan summer 2016
