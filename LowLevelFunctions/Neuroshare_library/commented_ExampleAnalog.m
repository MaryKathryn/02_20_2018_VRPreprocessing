...
% prompt to get the number of points (Think about your sample rate for
% continuous data)
...
%SDT : warning, this will pop up multiple figure windows !
if (yes == 'y')
    for i = 1 : 1 : length(AnalogList)
        chan = AnalogList(i);
        %Finds the smallest of the two value. This is the number of samples
        %you wish to see on plot
        count = min(max_count, EntityInfo(chan).ItemCount);
        % Get the first data points of the waveform and show it
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Look at individual analog channel
...
%Get some info on analog channel
[nsresult, nsAnalogInfo] = ns_GetAnalogInfo(hfile, channel);
